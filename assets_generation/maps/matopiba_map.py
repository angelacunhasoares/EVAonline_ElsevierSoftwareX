...
META_CSV_PATH = './data/csv/ST_PLOT.csv'  # CSV with meteorological station data (MA, TO, PI, BA)
BRASIL_GEOJSON_PATH = './data/geojson/BR_UF_2024.geojson'  # GeoJSON with Brazil's state boundaries
MATOPIBA_GEOJSON_PATH = './data/geojson/Matopiba_Perimetro.geojson'  # GeoJSON with MATOPIBA perimeter
CLIMATE_SHAPEFILE_PATH = './data/shapefile/clima_5000.shp'  # Shapefile with climate zones
CLIMATE_COLUMN_NAME = 'DESC_COMPL'  # Column in the shapefile with climate descriptions
OUTPUT_MAP_PATH = './frontend/assets/images/matopiba_map.png'  # Output map file
...
def add_scalebar(
    ax, metric_distance, bar_offset=(0.05, 0.05, 0.07), max_stripes=5, 
    bar_alpha=0.3, fontsize=28, linewidth=10
):
    ...
    """
    print(f"Adding scale bar to axis {ax.get_title()} with total length of {metric_distance / 1000} km and linewidth={linewidth}")
    ...
    bar_tickmark = metric_distance / max_stripes  # Distance in meters per segment
    bar_ticks = max_stripes
    bar_color = ['black', 'red']
    for i in range(bar_ticks):
        end_lat, end_lon = displace(bar_lat0, bar_lon0, 90, bar_tickmark)
        print(f"Drawing scale bar segment {i+1} with linewidth={linewidth}, from ({bar_lon0:.4f}, {bar_lat0:.4f}) to ({end_lon:.4f}, {end_lat:.4f})")
        ax.plot([bar_lon0, end_lon], [bar_lat0, end_lat], color=bar_color[i % 2],
                linewidth=linewidth, transform=ccrs.Geodetic(), solid_capstyle='butt', alpha=bar_alpha, zorder=20)
        bar_lon0 = end_lon
        bar_lat0 = end_lat
    buffer = [PathEffects.withStroke(linewidth=3, foreground="white")]
    total_distance_km = int(bar_ticks * bar_tickmark / 1000)
    ax.text(text_lon0, text_lat0, f"{total_distance_km} km",
            transform=ccrs.Geodetic(), horizontalalignment='left', verticalalignment='bottom',
            path_effects=buffer, zorder=21, fontsize=fontsize)
    print(f"Scale bar drawn: {total_distance_km} km with {max_stripes} segments of {bar_tickmark / 1000} km each")
...
try:
    ...
    gdf_brasil = gpd.read_file(BRASIL_GEOJSON_PATH).to_crs(epsg=4326)  # Brazil's state boundaries
    print("GeoJSON do Brasil carregado com sucesso.")

    print("Carregando GeoJSON do MATOPIBA...")
    gdf_matopiba = gpd.read_file(MATOPIBA_GEOJSON_PATH).to_crs(epsg=4326)  # MATOPIBA perimeter
    ...
    meta_df = pd.read_csv(META_CSV_PATH, sep=";", encoding='utf-8')  # Explicitly set UTF-8
    print("CSV de estações carregado com sucesso.")
    
    meta_df['CODE_ST'] = meta_df['CODE_ST'].astype(str)  # Converts station code to string
    meta_df = meta_df[meta_df['PLOT'] == 'YES']  # Filters stations for plotting
    ...
    colors = sns.color_palette("tab20", len(meta_df))  # Color palette for stations
    meta_df['color'] = colors
    gdf_stations = gpd.GeoDataFrame(
        meta_df, geometry=gpd.points_from_xy(meta_df.LONGITUDE, meta_df.LATITUDE), crs="EPSG:4326"
    )
    print(f"Dados carregados e processados. Total de estações com PLOT=YES: {len(gdf_stations)}")
...
gs_map_layout = gs_main_layout[0, 0].subgridspec(1, 2, width_ratios=[1, 1.3], wspace=0.15)
ax_context = fig.add_subplot(gs_map_layout[0], projection=PROJECTION)  # Axis for the Brazil map
ax_main = fig.add_subplot(gs_map_layout[1], projection=PROJECTION)  # Axis for the MATOPIBA map
...
border_patch = mpatches.Rectangle(
    ...
    linewidth=6, edgecolor='black', facecolor='none', transform=fig.transFigure, zorder=0
)
...
ax_context.add_feature(cfeature.OCEAN.with_scale('50m'), zorder=0)  # Adds ocean
ax_context.add_feature(cfeature.BORDERS.with_scale('50m'), linestyle='-', edgecolor='black', linewidth=0.7, zorder=3)  # Adds borders
...
for _, row in gdf_clima.iterrows():
    ...
    gpd.GeoSeries([row.geometry]).plot(ax=ax_context, color=color, alpha=0.8, transform=PROJECTION, zorder=1)

gdf_brasil.plot(ax=ax_context, facecolor='none', edgecolor='black', linewidth=0.5, zorder=10, transform=PROJECTION)  # State boundaries
gdf_matopiba.plot(ax=ax_context, facecolor='none', edgecolor='red', linewidth=1.5, transform=PROJECTION, zorder=4)  # MATOPIBA border
...
gl = ax_context.gridlines(draw_labels=True, dms=True, x_inline=False, y_inline=False, linestyle=':', color='black', alpha=0.4)
gl.top_labels = False
...
for spine in ax_main.spines.values():
    ...
    spine.set_linewidth(3.0)  # Black border thickness
    spine.set_edgecolor('black')
ax_main.add_feature(cfeature.OCEAN.with_scale('10m'), facecolor='#D6EAF8', zorder=0)  # Adds ocean

# Plots climate zones.
for _, row in gdf_clima.iterrows():
    color = get_color(row[CLIMATE_COLUMN_NAME])
    gpd.GeoSeries([row.geometry]).plot(ax=ax_main, color=color, alpha=0.8, transform=PROJECTION, zorder=1)

gdf_brasil.plot(ax=ax_main, facecolor='none', edgecolor='black', linewidth=0.5, zorder=10, transform=PROJECTION)  # State boundaries
gdf_matopiba.plot(ax=ax_main, facecolor='none', edgecolor='red', linewidth=6, linestyle='-', transform=PROJECTION, zorder=5)  # MATOPIBA border
...
for idx, row in gdf_stations.iterrows():
    ax_main.text(
        row.geometry.x, row.geometry.y + 0.08,  # Vertical offset
        str(row['number']),
        fontsize=26,
        ha='center',
        va='bottom',
...
ax_main.set_extent([minx - 6.5, maxx + 0.2, miny - 2.0, maxy + 1.2], crs=PROJECTION)
# Adds grid with latitude and longitude labels.
gl = ax_main.gridlines(draw_labels=True, dms=True, x_inline=False, y_inline=False, linestyle=':', color='black', alpha=0.4)
gl.top_labels = False
gl.right_labels = False
gl.xlabel_style = {'size': 28, 'color': 'black'}
gl.ylabel_style = {'size': 28, 'color': 'black'}
...
h_matopiba = [
    mpatches.Patch(edgecolor='black', facecolor='none', linewidth=3, label='State Boundaries'),
    mpatches.Patch(edgecolor='red', facecolor='none', linewidth=3, label='Matopiba Boundary')
]
h_cities = [
    Line2D([0], [0], marker='o', color='w', markerfacecolor=row['color'], markersize=30, linestyle='None',
...
ax_globe = fig.add_axes([0.12, 0.48, 0.12, 0.12], projection=ccrs.Orthographic(-60, -15))
ax_globe.add_feature(cfeature.OCEAN, facecolor='#D6EAF8', zorder=0)
ax_globe.add_feature(cfeature.BORDERS, linestyle='-')
ax_globe.add_feature(cfeature.LAND, facecolor='lightgray', edgecolor='black', zorder=1)
ax_globe.set_global()
gdf_brasil.plot(ax=ax_globe, facecolor='gray', edgecolor='black', linewidth=1.5, transform=PROJECTION, zorder=2)
gdf_matopiba.plot(ax=ax_globe, facecolor='none', edgecolor='red', linewidth=1.5, alpha=0.8, transform=PROJECTION, zorder=3)
...
con_kwargs = {"color": "black", "linewidth": 3, "arrowstyle": "->", "mutation_scale": 20}
con1 = ConnectionPatch(xyA=(minx, maxy + 0.5), xyB=(minx - 6.5, maxy + 1.2), coordsA=ax_context.transData, coordsB=ax_main.transData, zorder=40, **con_kwargs)
con2 = ConnectionPatch(xyA=(minx, miny - 0.5), xyB=(minx - 6.5, miny - 2.0), coordsA=ax_context.transData, coordsB=ax_main.transData, zorder=20, **con_kwargs)
fig.add_artist(con1)
fig.add_artist(con2)

# Adds scale bars to the Brazil (500 km) and MATOPIBA (100 km) maps.
print("Adding scale bar to MATOPIBA...")
add_scalebar(ax_main, metric_distance=100000, bar_offset=(0.05, 0.05, 0.07), max_stripes=5, bar_alpha=0.3, fontsize=30, linewidth=20)  # Note: linewidth=20 did not work as expected
print("Adding scale bar to Brazil...")
add_scalebar(ax_context, metric_distance=500000, bar_offset=(0.05, 0.05, 0.07), max_stripes=5, bar_alpha=0.3, fontsize=24, linewidth=10)


# Adds a compass rose to the Brazil map.
def add_compass(ax, pos_x, pos_y, size=0.1):
    ...
    ax_compass.arrow(0.5, 0.8, 0, -0.4, head_width=0.05, head_length=0.1, fc='black', ec='black', lw=1)
    ax_compass.arrow(0.5, 0.2, 0, 0.4, head_width=0.05, head_length=0.1, fc='black', ec='black', lw=1)
    ax_compass.arrow(0.2, 0.5, 0.4, 0, head_width=0.05, head_length=0.1, fc='black', ec='black', lw=1)
    ax_compass.arrow(0.8, 0.5, -0.4, 0, head_width=0.05, head_length=0.1, fc='black', ec='black', lw=1)
...
def create_legend(ax, handles, title):
    ...
    """
    legend = ax.legend(handles=handles, title=title, alignment="left", loc='upper left',
                       fontsize=32, title_fontsize=34, frameon=False, handleheight=1.5, labelspacing=0.8)
    legend.get_title().set_weight('bold')
    return legend


# Creates an external legend divided into three columns for climate zones.
gs_legend_layout = fig.add_gridspec(1, 3, left=0.03, right=0.97, bottom=0.05, top=0.30, width_ratios=[0.4, 0.5, 0.7], wspace=0.1)

# Adds a black border around the external legend.
legend_border = mpatches.Rectangle(
    (0.03, 0.05), 0.94, 0.25,
    linewidth=2, edgecolor='black', facecolor='none', transform=fig.transFigure, zorder=0
)
fig.add_artist(legend_border)

# Creates axes for the three legend columns and disables their visual elements.
ax_leg1 = fig.add_subplot(gs_legend_layout[0, 0]); ax_leg1.axis('off')
ax_leg2 = fig.add_subplot(gs_legend_layout[0, 1]); ax_leg2.axis('off')
ax_leg3 = fig.add_subplot(gs_legend_layout[0, 2]); ax_leg3.axis('off')

# Defines legend entries for climate zones (column 1).
zonas_handles = [
    mpatches.Patch(color=get_color('Equatorial, quente - média > 18° C em todos os meses, super-úmido sem seca'), label='Equatorial'),
    mpatches.Patch(color=get_color('Temperado, mesotérmico brando - média entre 10 e 15° C, super-úmido sem seca'), label='Temperate'),
    mpatches.Patch(color=get_color('Tropical Brasil Central, quente - média > 18° C em todos os meses, semi-árido 7 a 8 meses secos'), label='Tropical Central Brazil'),
    mpatches.Patch(color=get_color('Tropical Nordeste Oriental, quente - média > 18° C em todos os meses, semi-árido 9 a 10 meses secos'), label='Tropical Northeastern Brazil'),
    mpatches.Patch(color=get_color('Tropical Zona Equatorial, quente - média > 18° C em todos os meses, semi-árido 11 meses secos'), label='Tropical Equatorial Zone'),
]
...
legend2 = create_legend(ax_leg2, temp_umid_handles_col2, "Temperature and Humidity")
...
temp_umid_handles_col3 = [
    ...
    mpatches.Patch(color='#D9EAD3', label='subhot, semi-humid 4 to 5 dry months'),
    mpatches.Patch(color='#EFF6E9', label='subhot, semi-arid 6 dry months'),
    mpatches.Patch(color='#E6F5FB', label='mild mesothermal, semi-humid 4 to 5 months'),
]
legend3 = create_legend(ax_leg3, temp_umid_handles_col3, "Temperature and Humidity")

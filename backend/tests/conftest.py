...
def api_client_factory(sample_coordinates, sample_date_ranges):
    """Factory fixture for creating API clients with different configurations."""
    from backend.api.services.openmeteo import OpenMeteoForecastAPI

    def _create_client(location_key="jaú_sp", date_range_key="forecast_range"):
        lat, long = sample_coordinates[location_key]
        start, end = sample_date_ranges[date_range_key]
        return OpenMeteoForecastAPI(start, end, long, lat)

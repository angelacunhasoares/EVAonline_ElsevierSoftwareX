...
# Step 3.1: Validate weather data based on physical limits
@shared_task
def data_initial_validate(weather_df: pd.DataFrame, latitude: float) -> Tuple[pd.DataFrame, list]:
    """
    ...
    Args:
        weather_df (pd.DataFrame): Weather data with columns T2M_MAX, T2M_MIN, T2M, RH2M, WS2M, ALLSKY_SFC_SW_DWN, PRECTOTCORR.
        latitude (float): Latitude for calculating extraterrestrial radiation (Ra).

    Returns:
        Tuple[pd.DataFrame, list]: Validated DataFrame and list of warnings with metrics.
    ...
    if "date" not in weather_df.columns:
        warnings.append("The 'date' column is missing from the DataFrame.")
        logger.error(warnings[-1])
        return weather_df, warnings
    if not pd.api.types.is_datetime64_any_dtype(weather_df["date"]):
        warnings.append("The 'date' column must be in datetime format (YYYY-MM-DD).")
        logger.error(warnings[-1])
        return weather_df, warnings
    ...
    year = weather_df["date"].dt.year.iloc[0]
    total_days_in_year = 366 if is_leap_year(year) else 365

    # Calcular radiação extraterrestre (Ra)
    ...
    # Apply physical limits from Xavier et al. (2015)
    limits = {
        "T2M_MAX": (-30, 50, "left"),
        "T2M_MIN": (-30, 50, "left"),
        "T2M": (-30, 50, "left"),
    ...
    # Métrica: Percentual total de valores inválidos
...
# Step 3.2: Detect outliers using IQR
@shared_task
def detect_outliers_iqr(weather_df: pd.DataFrame, iqr_factor: float = 1.5) -> Tuple[pd.DataFrame, list]:
    """
    ...
    Returns:
        Tuple[pd.DataFrame, list]: DataFrame with outliers replaced by NaN and list of warnings with metrics.
    """
    logger.info("Detecting outliers with IQR")
    warnings = []

    weather_df = weather_df.copy()
    numeric_cols = [col for col in weather_df.columns if col not in ["date", "Ra", "dr", "delta", "omega_s"] and weather_df[col].dtype in [np.float64, np.int64]]
...
# Step 3.3: Impute missing weather data based on FAO-56 guidelines
@shared_task
def data_impute(weather_df: pd.DataFrame) -> Tuple[pd.DataFrame, list]:
    """
    ...
    Returns:
        Tuple[pd.DataFrame, list]: Imputed DataFrame and list of warnings with metrics.
    """
...
@shared_task
def preprocessing(weather_df: pd.DataFrame, latitude: float, cache_key: Optional[str] = None) -> Tuple[pd.DataFrame, list]:
    """
    Preprocessing pipeline: validation, outlier detection, and imputation.

    Args:
        weather_df (pd.DataFrame): Weather data.
        latitude (float): Latitude for Ra calculation.
        cache_key (Optional[str]): Key for caching results in Redis.

    Returns:
        Tuple[pd.DataFrame, list]: Preprocessed DataFrame and list of warnings with metrics.
    ...
    # Final validation: Ensure no NaNs remain
    if weather_df.isna().any().any():
        remaining_nans = weather_df.isna().sum().sum()
        percent_remaining = (remaining_nans / (len(weather_df) * len(weather_df.columns))) * 100
        warnings.append(
            f"Critical: {remaining_nans} NaN values remain ({percent_remaining:.2f}%) after preprocessing."
        )
        logger.error(warnings[-1])
        raise ValueError("Preprocessing failed: NaN values remain in the data.")
